...
`docs/solutions/` 保存了过往问题的可复用经验，按类别组织、带 YAML frontmatter（`module`、`tags`、`problem_type`、`domain` 等字段，可检索）。涵盖 bug 修复（`integration-issues/` 等）和团队约定/最佳实践（`conventions/`、`best-practices/` 等）。在文档已覆盖的领域做实现、调试或决策时值得先查一下——例如改微信发布 skill、写公众号文章前。新经验由 `$spec-compound` 沉淀至此。
