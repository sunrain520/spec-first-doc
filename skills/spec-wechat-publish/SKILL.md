...
**执行约定：** 本 skill 是 `文章系列/` 的仓库内运营资产，收纳在 `skills/spec-wechat-publish/`（与其他 skill 统一管理）。所有命令里的 `skills/spec-wechat-publish/...`（脚本）、`文章系列/pic/...`、`文章系列/...`（正文与配图）等均为仓库根目录的相对路径。**执行前确保当前工作目录是仓库根**（`spec-first-doc/`）。本 skill 不设计为复制到 `~/.claude/skills/` 当通用 skill 用——那样相对路径会全部失效。
...
test -f "skills/spec-wechat-publish/scripts/svg-to-png.mjs" \
  && echo "✓ svg-to-png.mjs" \
  || { echo "✗ svg-to-png.mjs 缺失"; MISSING+=("svg-to-png.mjs"); }

# 3. 微信封面上传脚本（必需）
test -f "skills/spec-wechat-publish/scripts/upload-wechat-cover.ts" \
...
node skills/spec-wechat-publish/scripts/patch-wechat-editor-selectors.mjs \
  --skill-dir "$HOME/.claude/skills/baoyu-post-to-wechat" \
  && echo "✓ 编辑器选择器补丁" \
...
- [ ] Step 6.6: 运营质量检查（软性：标题钩子/首屏/CTA/传播资产/受众）
...
```bash
npx -y bun skills/spec-wechat-publish/scripts/svg-to-png.ts \
  --batch 文章系列/pic/
```
...
> **标题运营软质量（硬格式之外）：** `Spec-First：` 前缀和无 H1 是上面的硬格式要求；但**冒号后那段写得好不好**直接决定打开率。理想是写读者痛点/收益（"为什么有人越改越顺、有人越改越崩"），而非作者成就（"我跑完了整个流程"）。这一项不在本步阻断，留到 Step 6.6 运营质量检查统一核（标准与正反例见 `references/operations-playbook.md`）。
...
## Step 6.6: 运营质量检查（软性，推送前）

**与 Step 4.7 / 5.5 的分工：** 4.7/5.5 是**硬 gate**（标题前缀、≤120 字摘要、≥1.5 万字、6-10 配图、版式），不过就拦。本步是**软质量**——gate 之外决定打开率/完读率/转化/传播的运营维度，不阻断发布，但发现缺口应提示用户、建议优化。

**读取：** `references/operations-playbook.md`（运营 playbook，含完整清单与正反例）。

**执行：** 对照 playbook 末尾「运营质量检查清单」逐条核，重点八项：

1. **标题**：冒号后是读者痛点/收益，不是作者成就？≤20 字、含具体数字/技术名、无夸大式紧迫？（`Spec-First：` 前缀已由 4.7 保证）
2. **首屏**：痛点故事开场、不是「导读+专名」劝退新读者？倒金字塔——只读首段就知道解决什么、给什么答案，钩子在前两屏？
3. **情感弹药**：画面感故事/翻车现场在开头出现过，没全囤在结尾？
4. **结尾 CTA**：关注引导 + 转发钩子 + 阅读原文话术 + 下一篇预告齐全，且**主次分明**（一个主 CTA + 动词收益措辞 + 降风险句）？
5. **连载承接**：下一篇预告点名具体问题/结果但不剧透？开头有一句"上集回顾"？
6. **传播资产**：有可截图的全景图/核心对比表/金句/结论卡？
7. **受众**：本篇锁定一类主受众，没同时服务三类？
8. **金句**：加粗金句可独立传播（脱离上下文仍有力）？

**处理：** 用 `AskUserQuestion` 汇报检查结果与发现的缺口，询问：
- 按建议优化后再推送（推荐，命中缺口时）
- 接受现状直接推送（运营软质量不达标也不强制阻断）

短钩子/BIP 等非主干正文：标题前缀、字数、配图要求放宽，但开篇钩子与 CTA 原则仍适用，按 playbook 的豁免说明处理。
...
- **运营质量 playbook（Step 4.7 标题运营 + Step 6.6 运营检查用）**：[references/operations-playbook.md](references/operations-playbook.md)（标题钩子/开篇四步+NN/g 倒金字塔/CTA 主次/连载追更/打开率·完读率·转化·传播原则/读者分层；完整经验见 `docs/solutions/conventions/wechat-article-operations-playbook-2026-06-15.md`）
