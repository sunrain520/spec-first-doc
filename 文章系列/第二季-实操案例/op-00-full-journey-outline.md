---
name: op-00-full-journey-outline
description: 第二季（项目实操）第 0 篇大纲：用一个真实需求走完 spec-first 完整链路，建立全局地图；包含三种研发模式（0-1/1-10/10-100）、三种仓库拓扑（单仓单项目/单仓多模块/多仓工作区）和 spec-prd 的位置
metadata:
