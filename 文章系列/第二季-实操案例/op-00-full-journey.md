...
title: Spec-First：同样用 AI 写代码，为什么有人越改越顺、有人越改越崩
...
**同样用 AI 写代码，有人越改越顺，有人越改越崩。差别往往不在 AI，在你站在了哪个位置。**

先讲两个我真实见过的卡壳现场。

一个人想做个全新的小工具，方向还很模糊，但他直接上来就让 AI 写详细计划——结果计划里全是"假设"，因为需求本身都没想清楚，计划只能建在沙子上，写完发现方向就是错的，整份作废。

另一个人在一个跑了三年的老系统上加功能，他像做新项目一样让 AI"自由发挥"，结果 AI 不知道这个系统的历史约束，改一处崩三处——因为存量系统最大的风险不是不会写，是不知道"原来这里不能动"。

这两个人都很努力，也都信了"AI 能帮我写代码"。但他们一个把小事做崩、一个把老系统改崩，**不是方法用错了，是一开始就站错了位置**——他们默认"所有需求都该用同一套流程跑"。

而站对位置之后是什么体感？我用 spec-first 开发 spec-first 本身已经超过半年。有一次我隔了快两个月回去改一个模块，完全忘了当初为什么那样设计——换作以前我得花半天重新读代码猜意图，但这次我打开两份文档，当初的需求边界、计划取舍、踩过的坑全在那里，十分钟就回到了状态。还有一次我把一个任务从 Claude Code 换到 Codex 继续做，新的 AI 不需要我重新解释一遍，读文档就接上了。

**同样是用 AI 写代码，区别就在这里：有人每次都从零开始，有人每次都站在上一次的肩膀上。** 这篇文章讲的，就是怎么站对位置。

> **导读**
> 这是 spec-first 系列第二季的开篇。第一季建立了"为什么需要给 AI 编程套一层方法框架"的认知，没读过也不影响——这篇会从零带你建立全局视角。第二季从这里开始讲"怎么用"：先给你两张地图，让你知道自己在哪里、该走哪条路。

先把全局放在这里——这一张图就是这篇文章的全部骨架，看不懂没关系，读完再回来看它会变得很清晰。**建议先截图存下，下次开工前瞄一眼就知道自己该走哪条路。**

![spec-first 全景一图：两张地图 + 主链 + 三配套](../pic/op-00-full-journey-overview.png)

---

## 01 为什么需要两张地图

回到开头那两个卡壳的人。全新产品和存量系统，本就该走不同的路。硬套同一套流程，要么是给小事配重型仪式，要么是给重活省掉关键防护——两个方向都会出问题。
...
开头我讲了两个画面：隔两个月回来十分钟回到状态、换个 AI 读文档就接上。走完整张地图，现在你应该能看懂这两个画面背后是什么了——它们不是 spec-first 多了什么魔法，而是链路里每个节点都把上下文**落成了文件**：需求落在 `docs/brainstorms`、计划落在 `docs/plans`、经验落在 `docs/solutions`。于是"未来的我"和"接手的人"不用靠记忆和猜测，直接读文件就行。

最开始，我也觉得这些步骤有点繁琐。明明一句话能让 AI 开干的事，为什么要先 brainstorm 再 plan 再 review？但随着项目变大，体感越来越清楚：**繁琐的是"每次都从零开始"，不是"先花十分钟把地图看清"。**

再补一个开头没讲的维度——团队。这套链路单人用是"对抗自己的遗忘"，多人用是"对抗团队的失对齐"：新人来了不靠口耳相传了解历史决策，他读 `docs/` 就知道这个系统为什么长这样、哪些坑别人替他踩过了。项目活得越久、经手的人越多，这份沉淀的复利越大。
...
**第二季会把这套方法在每个真实场景里完整跑一遍，怕错过的话，关注一下不迷路。** 如果你身边也有人"用 AI 写代码越改越乱"，把这张地图转给他——少走的弯路，比任何说教都有用。
...
想上手的话，最低成本的第一步是先读下一篇 op-01——跟着把一个真实功能从头跑一遍，比直接装工具更容易建立手感。想直接看代码或装来试，文末「阅读原文」直达 GitHub。
