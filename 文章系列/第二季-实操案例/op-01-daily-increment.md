...
description: spec-first 微信公众号第二季（项目实操）首个完整案例：承接总览篇，用最常见的日常增量场景——给待办应用加一个标签过滤功能，把 spec-first 完整链路从一句话需求跑到经验沉淀，每步给出输入、产物和判断。
metadata:
  type: article
  series: op
  series_index: "op-01"
...
上一篇总览（op-00）已经画好了两张地图、把完整链路示意了一遍。从这一篇起，我们落到地面：不讲抽象流程，挑一个最普通的日常需求，从头到尾跑一遍给你看。
...
总览篇（op-00）画的那"两张地图"，这里再快速对一下坐标：一张是**需求模式**（0-1 全新 / 1-10 增量 / 10-100 存量改造），一张是**仓库拓扑**（单仓单项目 / 单仓多模块 / 多仓工作区）。任何一个需求，都先在这两张地图上定位，再决定走哪条链路。
...
![op-01 brainstorm 收敛：一句模糊需求经一问一答，逐个补齐边界/约束/禁区，收敛成 requirements](../pic/op-01-brainstorm-funnel.png)
...
![op-01 本例链路走查：第零步到第七步每步的动作与产物，灰色节点本例跳过](../pic/op-01-chain-walkthrough.png)
...
2. **task identity**：确认这次执行对的是当前的需求和计划，没跑在过期的链路上。（像我们这次跳过了任务包，work 直接从 plan 跑、靠 plan 的 `spec_id` 锚定；只有走了 write-tasks 的大需求，才会多一道 `source_plan_hash` 校验任务包和计划还对不对得上。）
...
它先算出本次 diff 的范围、跑一遍运行时就绪检查、收集测试结果作为证据，然后**按这次改动的规模和风险，挑选合适的 reviewer 并行**——常见的有正确性、测试覆盖、可维护性、项目规范，碰到安全、性能、接口契约相关的改动，再加挂对应的专项 reviewer。改动越小越安全，上的 reviewer 越少；像我们这个三文件的小改动，几个核心 reviewer 就够了。
...
```text
docs/solutions/architecture-patterns/tag-filter-grouping-coexistence-2026-06-14.md
```
...
![op-01 四类翻车，被链路哪一步接住：缺边界/缺既有约束/缺禁区/没测试，分别由 brainstorm、plan+compound、work、code-review 接住](../pic/op-01-failure-caught.png)
...
**第二季后面还有六个真实场景——从 0 到 1 新产品、老系统改造、多仓协作、出错怎么收拾、换人接手、上线把关——一篇一个，关注一下不迷路。** 如果你身边也有人"一句话甩给 AI 然后反复返工"，把这篇转给他——看一遍真实链路，比讲十遍道理管用。

想上手的话，最低成本的第一步就是挑一个你手上最普通的小需求，照着这条链路跑一遍；spec-first 是开源的、装上就能用，想直接看代码或装来试，文末「阅读原文」直达 GitHub。

下一篇，我们把场景换一换：如果不是给已有产品加功能，而是从 0 到 1 做一个方向都还没定的新东西，spec-first 又是怎么帮你从模糊想法收敛到能动手的？
