---
name: spec-first-wechat-003-series-framework
description: spec-first 微信公众号系列总框架——分层框架、三大内容流、初期标题池（含 Harness 定位）
metadata:
  type: project
---

# spec-first 微信公众号 · 003 系统总框架
...
> **标题格式约定：** 系列正文标题统一为 `Spec-First：xxxx`，下表候选已按此格式给出；`xxxx` 同时承载该篇核心论点，写作时可微调措辞但保留前缀。完整格式说明见 `004-第一季路线图.md`。
...
- 已发布文章：第一季（设计思想）见 `第一季-harness认知/`，第二季（项目实操）见 `第二季-实操案例/`，第三季（细节设计）见 `第三季-skill深度剖析/`
- 草稿建议在对应季目录用 `02-`、`op-`、`s3-` 等前缀顺序存放
- 文章配图统一存入顶层共享目录 `pic/`
