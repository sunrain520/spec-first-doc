---
name: spec-first-wechat-004-s1-roadmap
description: spec-first 微信公众号第一季路线图：12 篇最小叙事包、每周三篇结构、证据票据与可回流资产（历史规划，实际收窄为 5 篇）
metadata:
  type: project
---

# spec-first 微信公众号 · 004 第一季路线图

**状态：** 历史规划稿（第一季实际收窄为 5 篇，详见下方演化说明）
**日期：** 2026-05-27
**当前基线：** 第一季已发布 01-05 共 5 篇，已收官；发布台账见 `已发布台账.md`
**关联规划：** `运营规划/003-系统总框架.md`；顶层骨架与三季对账见 `运营规划/006-顶层总规划.md`
**Ideation 记录：** `docs/ideation/2026-05-27-spec-first-wechat-series-content-ideation.md`

> **演化说明（必读）：** 本文是 2026-05-27 的第一季初始规划，列了 12 篇（实列 11 篇，跳号缺 06）。实际第一季只发布 01-05 共 5 篇即收官——原计划的「操作型分篇」（07 Spec / 08 Plan / 09 Scripts-vs-LLM / 10 Review / 11 Knowledge / 12 端到端）全部被第二季的 skill 深度剖析吸收升级，不再作为第一季单独发布。完整去向对账见 `006` 总规划第二节。下方 12 篇目录与资产节奏按历史规划保留，不再据此追加发布 06-12。
